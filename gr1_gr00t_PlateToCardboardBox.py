...
def main():
    ...
    bin: RigidPrim = scene.add(XFormPrim(
        prim_paths_expr="/World/bin",
        name = "bin",
        scales=np.array([[1.0, 1.2, 0.4]]),
        visibilities=np.array([True,]),
        #masses=[2.0, ]
    ))
    ...
    PROMPT = "pick the green cube and place it in the cardboard box"
    ...
    for simulation_num in range(5):
        ...
        table.set_world_poses(positions=np.array([[0.55,0,-0.0]]),
                            orientations=rot_utils.euler_angles_to_quats(np.array([0, 0, 90.0]), degrees=True).reshape((1,4)))

        object.set_world_pose(np.array([0.33 + 0.1 * (np.random.rand() - 0.5), -0.15 +  0.05 * (np.random.rand() - 0.5), 1.1]))
        bin.set_world_poses(positions=np.array([[0.4,-0.5,1.05]]),
                            orientations=rot_utils.euler_angles_to_quats(np.array([0, 0, 90.0]), degrees=True).reshape((1,4)))
